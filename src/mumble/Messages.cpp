...
void MainWindow::msgTextMessage(const MumbleProto::TextMessage &msg) {
	...
	if (g.s.bTTSNoScope || g.s.bTTSNoAuthor) {
		if (g.s.bTTSNoScope && g.s.bTTSNoAuthor){
			shortTTS += tr("%2%1: %3").arg(QString()).arg(QString()).arg(u8(msg.message()));
		}else if (g.s.bTTSNoAuthor){
			shortTTS += tr("%2%1: %3").arg(QString()).arg(target).arg(u8(msg.message()));
		}else if (g.s.bTTSNoScope){
			shortTTS += tr("%2%1: %3").arg(name).arg(QString()).arg(u8(msg.message()));
		}
	}
	...
	g.l->log(privateMessage ? Log::PrivateTextMessage : Log::TextMessage,
	         ...
	         false,
	         shortTTS.isNull() ? QString() : shortTTS);
}
